...
class QMix(nn.Module):
    ...
    def forward(self, states):
        ...
        print(q_values.shape, hyper_w1.shape, hyper_b1.shape)
        hidden = torch.relu(torch.bmm(q_values, hyper_w1) + hyper_b1)
        hyper_w2 = torch.abs(self.hyper_network_weight_2(hyper_state).view(-1, hidden.size(-1), 1))  # (batch_size, hidden_dim, 1)
        hyper_b2 = torch.nn.relu(self.hyper_network_bias_2_1(hyper_state).view(-1, 1, 1))  # (batch_size, 1, 1)
        hyper_b2 = self.hyper_network_bias_2_2(hyper_b2).view(-1, 1, 1)  # (batch_size, 1, 1)
